...
def prepare_parser():
  ...
  parser.add_argument(
    '--E1_fea_w', type=dict, default={4:.1, 8:.1, 16:.1, 32:.1},
    help='We sum feature of Encoder with generator, and use parameters to blance terms. The order is 4:8:16:32')
  parser.add_argument(
    '--D_fea_w', type=dict, default={4:0.1, 8:0.1, 16:0.1, 32:.01},
    help='We compute the distance between real image and fake image, to keep the structure information. The order is 4:8:16:32')
  parser.add_argument(
  ...
  parser.add_argument(
    ...
    help='Number of samples to compute inception metrics with '
         '(default: %(default)s)')
  parser.add_argument(
    '--N_target_cate', type=int, default=149,
    help='Number of target dataset '
         '(default: %(default)s)')
  parser.add_argument(
    '--model_ite', type=int, default=0,
    help='Saved model on specifial iteration (default: %(default)s)')
  ...
  parser.add_argument(
    '--base_root', type=str, default='/DATA/data/Biggan_pytorch/good_original_data',
    help='Default location to store all weights, samples, data, and logs '
           ' (default: %(default)s),/DATA/data/Biggan_pytorch/single_cate/n01440764_hdf, /DATA/data/Biggan_pytorch/good_original_data')
  parser.add_argument(
  ...
  parser.add_argument(
    '--test_root', type=str, default='',
    help='Default location to store test samples (default: %(default)s)')  
  parser.add_argument(
  ...
  parser.add_argument(
    ...
    help='Suffix for which weights to load (e.g. best0, copy0) '
         '(default: %(default)s)')
  parser.add_argument(
    '--training_scratch', action='store_true', default=False,
    help='training model from scratch (default: %(default)s)')
  parser.add_argument(
    '--resume', action='store_true', default=False,
    help='Resume training? (default: %(default)s)')
  parser.add_argument(
    '--resume_BigGAN', action='store_true', default=False,
    help='Resume the pre-trained BigGAN? (default: %(default)s)')
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64, 
                     ...
                     num_epochs=500, use_multiepoch_sampler=False, target_domain=None, **kwargs):

  # Append /FILENAME.hdf5 to root if using hdf5
  if target_domain is not None:
      data_root = target_domain 
...
def update_config_roots(config):
  if config['base_root']:
    ...
    for key in ['data', 'weights', 'logs', 'samples', 'test']:
        if config['%s_root' % key] =='':#  No given values or paths 
...
def prepare_root(config):
  for key in ['weights_root', 'logs_root', 'samples_root', 'test_root']:
...
def save_weights(G, D, E1, A1, state_dict, weights_root, experiment_name, 
                 name_suffix=None, G_ema=None, copy=False):
  if copy:
  ...
  torch.save(E1.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['E1', name_suffix])))
  torch.save(E1.optim.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['E1_optim', name_suffix])))
  torch.save(A1.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['A1', name_suffix])))
  torch.save(A1.optim.state_dict(), 
              '%s/%s.pth' % (root, join_strings('_', ['A1_optim', name_suffix])))
...
def load_weights(G, D, E1, A1, state_dict, weights_root, experiment_name, 
                 name_suffix=None, G_ema=None, strict=True, load_optim=True, resume_BigGAN =False, model_ite=0):
  if  resume_BigGAN: 
    print('Loading the pre-trained BigGAN...' )
    root = './BigGAN_weights'
  else:
    print('Loading the current model...' )
    root = '/'.join([weights_root, experiment_name])
    if  model_ite>0: # it is used to test the saved model on special iteration
      root = '/'.join([weights_root, experiment_name, str(model_ite)])
  ...
  if  resume_BigGAN: 
    if E1 is not None:
      E1.load_state_dict(
        ...
        strict=strict)
      if load_optim:
        E1.optim.load_state_dict(
          torch.load('%s/%s.pth' % (root, join_strings('_', ['D_optim', name_suffix]))))
  else:
    if E1 is not None:
      E1.load_state_dict(
        torch.load('%s/%s.pth' % (root, join_strings('_', ['E1', name_suffix]))),
        strict=strict)
      if load_optim:
        E1.optim.load_state_dict(
          torch.load('%s/%s.pth' % (root, join_strings('_', ['E1_optim', name_suffix]))))
    if A1 is not None:
      A1.load_state_dict(
        torch.load('%s/%s.pth' % (root, join_strings('_', ['A1', name_suffix]))),
        strict=strict)
      if load_optim:
        A1.optim.load_state_dict(
          torch.load('%s/%s.pth' % (root, join_strings('_', ['A1_optim', name_suffix]))))
  # Load state dict
...
def sample(G, M, E1, A1, z_, y_,loaders_t, t_data, config):
  with torch.no_grad():
    z_.sample_()
    y_.sample_()
    try:
      ...
      t_batch = next(t_data)# : target domain
      t_x, t_y = t_batch
    # here t_x has more data than z_
    t_x_, t_y_ = t_x[torch.randint(0, t_x.shape[0], (z_.shape[0],))], t_y[torch.randint(0, t_x.shape[0], (z_.shape[0],))]
    #fixed_t_x, fixed_t_y = t_x[:G_batch_size].detach().clone(), t_y[:G_batch_size].detach().clone()
    t_x_, t_y_ = t_x_.to('cuda'),  t_y_.to('cuda')

    if config['parallel']:
      #z_from_M, y_from_M = M(z_)
      z_from_M, y_from_M =  nn.parallel.data_parallel(M, (z_,))
      E1_L_feat = E1(t_x_, t_y_, E1_output_feat = True)
      E1_L_feat = A1(E1_L_feat)
      G_z =  nn.parallel.data_parallel(G, (z_, G.shared(y_), E1_L_feat))
      ...
      E1_L_feat = E1(t_x_, t_y_, E1_output_feat = True)
      E1_L_feat = A1(E1_L_feat)
      G_z = G(z_, G.shared(y_), E1_L_feat)
      #G_z = G(z_from_M, y_from_M)
      #G_z = G(z_, G.shared(y_))
    return G_z, y_

# Sample function for sample sheets
def sample_sheet(G, E1_L_feat, classes_per_sheet, num_classes, samples_per_class, parallel,
  ...
  for i in range(num_classes // classes_per_sheet):
    ...
    for j in range(samples_per_class):
      ...
      with torch.no_grad():
        if parallel:
          if classes_per_sheet != E1_L_feat[32].shape[0]:
              E1_L_feat = {keys: E1_L_feat[keys][:classes_per_sheet] for keys in E1_L_feat.keys()}
          o =  nn.parallel.data_parallel(G, (z_[:classes_per_sheet], G.shared(y), E1_L_feat))
          #o =  nn.parallel.data_parallel(G, (z_from_M, y_from_M, E1_L_feat))
          #o = nn.parallel.data_parallel(G, (z_[:classes_per_sheet], G.shared(y), E1_L_feat))
        else:
          if classes_per_sheet != E1_L_feat[32].shape[0]:
              E1_L_feat = {keys: E1_L_feat[keys][:classes_per_sheet] for keys in E1_L_feat.keys()}
          o = G(z_[:classes_per_sheet], G.shared(y), E1_L_feat)
          #o = G(z_from_M, y_from_M, E1_L_feat)
...
# Sample function for sample sheets
def sample_sheet_all_cate(G, E1_L_feat, classes_per_sheet, num_classes, samples_per_class, parallel,
                 samples_root, experiment_name, folder_number, z_=None, num_target_cate=None, index_to_class=None, index_epoch_num=0):
  # Prepare sample directory
  num_classes = num_target_cate # birds 
  # loop over total number of sheets
  for i in range(num_classes):
    ims = []
    #y = torch.arange(i * classes_per_sheet, (i + 1) * classes_per_sheet, device='cuda')
    classes_per_sheet=E1_L_feat[32].shape[0]
    y = torch.arange(i * classes_per_sheet, (i + 1) * classes_per_sheet, device='cuda')
    y = torch.ones_like(y)*0 + i
    for j in range(samples_per_class):
      if (z_ is not None) and hasattr(z_, 'sample_') and classes_per_sheet <= z_.size(0):
        z_.sample_()
      else:
    ...
    for class_name in index_to_class.values():
        if not os.path.exists('%s/%s/%s' % (samples_root, experiment_name,  class_name)):
            os.makedirs('%s/%s/%s' % (samples_root, experiment_name, class_name))


    for index_, label in enumerate(y):
        image_filename = '%s/%s/%s/%d.jpg' % (samples_root, experiment_name, 
                                                     index_to_class[int(label.cpu().numpy())], index_epoch_num*classes_per_sheet + index_)
        torchvision.utils.save_image(out_ims[index_], image_filename,
                                 nrow=samples_per_class, normalize=True)
...
def prepare_z_y(G_batch_size, dim_z, nclasses, device='cuda', 
                fp16=False,z_var=1.0, N_target_cate = 100):
  ...
  y_.init_distribution('categorical',num_categories=nclasses, N_target_cate =N_target_cate)
