...
def run(config):
  ...
  if config['resume']:
    ...
    utils.load_weights(G, D, E1, A1, state_dict,
                       ...
                       G_ema if config['ema'] else None, resume_BigGAN =config['resume_BigGAN'])#  :I add load_optim=Fasle
  ...
  #for epoch in range(state_dict['epoch'], 7):    
    ...
    if config['pbar'] == 'mine':#  here it is 'mine' 
   ...
   # pdb.set_trace()
    for i, (x, y) in enumerate(pbar):
      ...
      if  state_dict['itr'] > 138500 or config['training_scratch']: # the number 138000 is the last interation of the pre-trained BigGAN, here we train the adaptor 500 iterations
          stage = 2
      else:
          stage = 1
      ...
      if not (state_dict['itr'] % config['save_every']):
        if config['G_eval_mode']:
          ...
          if config['ema']:
            G_ema.eval()
        train_fns.save_and_sample(G, D, E1, A1, G_ema, z_, y_, fixed_z, fixed_y, 
