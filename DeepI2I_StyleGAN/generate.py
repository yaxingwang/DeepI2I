...
if __name__ == '__main__':
    ...
    parser.add_argument('--saved_path', type=str, default='test', help='path to save the generated images')
    ...
    utils.save_image(img, '%s/sample.png'%args.saved_path, nrow=args.n_col, normalize=True, range=(-1, 1))
    
    for j in range(20):
        img = style_mixing(generator, step, mean_style, args.n_col, args.n_row, device)
        utils.save_image(
            img, f'%s/sample_mixing_{j}.png'%args.saved_path, nrow=args.n_col + 1, normalize=True, range=(-1, 1)
        )
